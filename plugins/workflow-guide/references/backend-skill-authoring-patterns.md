...
---
name: entity-skill
description: JPA Entity와 Repository를 프로젝트 컨벤션에 맞게 새로 만들거나 수정할 때 사용. scalar FK, 관계 어노테이션 지양, 연결 엔티티, catalog 선택, Repository 메서드, QueryDSL 필요 여부를 함께 판단한다.
---
```
...
5. Repository와 필요한 CustomRepository/QueryDSL 명시 조인 작성
6. 보안/인가, 쿼리 병목, 트랜잭션 경계 확인
7. 컴파일/테스트 실행
...
| 단일 도메인의 정책, 검증, 상태 변경, 저장 | `UserService`, `PostService` | Repository, port, adapter abstraction 의존 가능 |
