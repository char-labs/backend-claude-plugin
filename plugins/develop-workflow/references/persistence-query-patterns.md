...
## Repository 포트와 CoreRepository 어댑터

- `*Repository`는 도메인/application 포트 인터페이스다. Spring Data `JpaRepository`, QueryDSL, `EntityManager`, persistence entity type을 직접 상속하거나 노출하지 않는다.
- `*CoreRepository`는 infrastructure/storage 구현체다. `class UserCoreRepository(...) : UserRepository`처럼 `*Repository`를 구현하고, 내부에서 `*JpaRepository`, `*CustomRepository`, QueryDSL/SQL adapter를 조합한다.
- `*JpaRepository`는 Spring Data 전용 interface로 infrastructure package에 둔다. Service/use case/Facade가 `*JpaRepository`나 `*CoreRepository`를 직접 주입받으면 계층 경계 finding으로 본다.
- Custom query가 필요하면 `*CustomRepository` 또는 QueryDSL helper를 `*CoreRepository` 뒤에 숨긴다. caller에게는 business read/write 의도가 드러나는 `*Repository` method, `*Query`, `*Criteria`, `*Result`만 노출한다.
- JPA Entity는 infrastructure/db-core/persistence adapter에 둔다. domain에는 JPA annotation이 없는 순수 data class 객체를 두고, `*CoreRepository`가 Entity `toDomain()` 변환으로 domain/result mapping을 소유한다.
- `*CoreRepository`는 Entity와 domain/result mapping을 소유하되 인가 결정, workflow state machine, transaction orchestration을 숨기지 않는다. 그런 책임은 Service/use-case에 둔다.
...
- Pida-Server식 구조를 따르는 프로젝트에서는 `*Repository`는 추상화된 포트, `*CoreRepository`는 구현체, `*JpaRepository`는 내부 위임 대상으로 유지한다.
- Pida-Server식 구조를 따르는 프로젝트에서는 JPA Entity를 domain module에 두지 않고 infrastructure/db-core에서 관리하며, domain에는 순수 data class와 `*Repository` 포트만 둔다.
