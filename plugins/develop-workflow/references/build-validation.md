...
- Gradle module은 편집 파일에서 root 방향으로 가장 가까운 `build.gradle.kts` 또는 `build.gradle`을 찾아 추정한다.
- Kotlin 테스트 파일 변경은 가능한 경우 가장 가까운 module의 `compileTestKotlin`으로 먼저 검증한다.
...
- Kotlin test compile: `./gradlew :module:compileTestKotlin`.
