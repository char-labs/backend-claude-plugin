...
description: 읽기 전용 종합 백엔드 리뷰 에이전트. PR, diff, 코드리뷰, 감사, 머지 전 검토, 품질 게이트에서 아키텍처, OOP/SOLID, 보안, 성능, 테스트, scalar FK/JPA 관계 어노테이션을 함께 볼 때 사용. 보안만 보면 security-reviewer, 시스템 성능은 performance-reviewer, 쿼리 전용 리뷰는 persistence-query-specialist, 설계/OOP 전용 리뷰는 oop-solid-reviewer를 사용.
