...
description: API 계약 설계 에이전트. GraphQL, gRPC/protobuf, REST/OpenAPI, 요청/응답 DTO, 스키마 변경, 필드 추가, 하위 호환성, 페이지네이션, 오류 응답, API 보안 경계 설계 시 사용. 실제 코드 구현은 backend-coder, 테스트 작성은 backend-test-writer를 사용.
...
- contract compatibility와 authorization을 검증하는 테스트를 포함합니다.
