...
ROUTES: list[Route] = [
    ...
    Route("kotlin", "kotlin-specialist", "kotlin-backend-workflow", "Kotlin/JVM idiom, nullability, scope function, sealed/value/data class, extension function, Flow/StateFlow, Gradle module validation, compileTestKotlin 범위 설계·구현·리뷰"),
...
CATEGORY_PATTERNS: list[tuple[str, list[str]]] = [
    ...
    ("kotlin", [r"\bkotlin\b", r"\bnullability\b", r"\bscope function(s)?\b", r"\bsealed (class|interface)\b", r"\bvalue class\b", r"\bdata class\b", r"\bextension function(s)?\b", r"\bstateflow\b", r"\bsharedflow\b", r"\bidiomatic kotlin\b", r"\bcompiletestkotlin\b", r"코틀린|Kotlin|널\s*안전|nullability|scope\s*function|스코프\s*함수|sealed\s*(class|interface)|sealed\s*클래스|value\s*class|값\s*클래스|data\s*class|데이터\s*클래스|확장\s*함수|extension\s*function|StateFlow|SharedFlow|idiomatic\s*Kotlin|코틀린답게|compileTestKotlin"]),
