#!/usr/bin/env python3
"""Advisory hook output for develop-workflow."""
...
def post_tool(data: dict) -> None:
    ...
    if entityish(path):
        ...
        return
    emit(
        "Develop workflow 안내: 백엔드 변경이라면 인가/권한, 입력 검증, 트랜잭션 경계, 쿼리 형태, 오류 처리, 민감정보 로깅을 확인하세요. 테스트는 비즈니스 로직이나 회귀 위험이 있을 때 집중적으로 작성하세요."
    )


def stop() -> None:
    emit(
        "Develop workflow 종료 안내: 백엔드 동작이 바뀌었다면 최종 응답에 수행한 검증과 남은 보안/성능/아키텍처 리스크를 명시하세요."
    )
