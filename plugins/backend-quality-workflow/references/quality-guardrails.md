...
이 문서는 backend-quality-workflow의 모든 skill과 agent에 자연스럽게 적용되는 실수 방지 원칙이다. 별도 workflow로 호출하기보다 설계, 구현, 리뷰, 테스트, 빌드, Git 작업 안에서 기본 검증 습관으로 사용한다.
...
```bash
backend-claude-plugin/plugins/backend-quality-workflow/tests/run-guardrail-policy-tests.py
backend-claude-plugin/plugins/backend-quality-workflow/tests/eval/routing/run-routing-tests.py
backend-claude-plugin/plugins/backend-quality-workflow/tests/run-hook-tests.py
claude plugin validate backend-claude-plugin
claude plugin validate backend-claude-plugin/plugins/backend-quality-workflow
```
