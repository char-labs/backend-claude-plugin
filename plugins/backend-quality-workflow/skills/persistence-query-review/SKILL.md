...
description: Repository, SQL, JPQL, QueryDSL, JPA/Hibernate, fetch strategy, N+1 수정, pagination, index, projection, transaction boundary 등 쿼리 중심 백엔드 작업을 리뷰하거나 설계할 때 사용.
...
3. N+1, lazy load, unbounded read, pagination 누락, broad fetch, index 누락, expensive count를 확인한다.
4. transaction scope, per-row write, batch behavior, stale persistence context risk를 확인한다.
5. query result correctness, authorization boundary, pagination/limit, bottleneck regression 테스트를 제안한다.
