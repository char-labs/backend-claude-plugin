#!/usr/bin/env python3
"""UserPromptSubmit router for backend-quality-workflow.

Emits a compact routing protocol only for backend-quality work. It avoids
...
ROUTES: list[Route] = [
    ...
    Route("query", "persistence-query-specialist", "persistence-query-review", "Repository, SQL/JPQL/QueryDSL, JPA fetch, N+1, 인덱스, 페이지네이션, 데이터 접근 권한 등 영속성/쿼리 작업"),
    ...
    Route("skill-authoring", "backend-architect", "backend-skill-authoring", "백엔드 컨벤션, 개발 절차, Entity/Repository/API/Test 패턴을 SKILL.md와 reference로 정리하는 작업"),
...
def main() -> None:
    ...
    print(
        f"""지시: BACKEND QUALITY ROUTING PROTOCOL
...
Step 1 - 사용 가능한 backend-quality 에이전트를 평가:
{agent_lines()}
